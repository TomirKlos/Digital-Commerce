var ProductComponent = function(data) {
    var template = [
        ...
        '__price__ PLN',
        '</span>',
        '<a data-id="__id__" name="__name__" class="product__add-to-basket btn btn-primary btn-sm float-right" href="#" role="button">Add to basket</a>',
        '</div>',
        '</div>',
        '</div>',
    ...
    Array.from(productList.getElementsByClassName("product__add-to-basket"))
        .forEach(function(element){
            element.addEventListener('click', function(e) {
                e.preventDefault();
                handler.addProduct(this.getAttribute('data-id'), this.getAttribute('name'));
